...
fn test_blind_sign_request() -> CredxResult<()> {
    ...
    let res = check_link_secret_equality(
        CRED_ID, &issuer_public_1, &mut issuer_1,
        &issuer_public_2, &mut issuer_2,
        &blind_claims_1, &blind_claims_1, // <-- same blind claim for both credentials
        nonce);
    if let Ok((presentation, pres_schema)) = res {
        presentation.verify(&pres_schema, &nonce)?;
    } else {
        return Err(Error::General("presentation.verify should succeed on equal link secrets"))
    }
    ...
    let res = check_link_secret_equality(
        CRED_ID, &issuer_public_1, &mut issuer_1,
        &issuer_public_2, &mut issuer_2,
        &blind_claims_1, &blind_claims_2, // <-- different blind claims
        nonce);
    if let Err(Error::InvalidClaimData("equality statement - claims are not all the same")) = res {
        ...
        Err(Error::General("Presentation::create should have failed on non equal link secrets"))
    }
}
